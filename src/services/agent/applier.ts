...
export async function applyToJob(jobId: string, userId?: string, dryRun: boolean = true): Promise<boolean> {
  ...
  let profile;
  if (userId) {
    profile = await prisma.userProfile.findFirst({ where: { userId } });
  } else {
  ...
  let settings;
  if (userId) {
    settings = await prisma.agentSettings.findFirst({ where: { userId } });
  } else {
    settings = await prisma.agentSettings.findFirst();
  }
  ...
  try {
    ...
    if (settings?.cookiesJson) {
      try {
        const decryptedCookiesJson = decryptString(settings.cookiesJson);
