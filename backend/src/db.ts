...
export async function logSystem(level: 'INFO' | 'WARNING' | 'ERROR' | 'SUCCESS' | 'SECURITY', message: string, payload?: any) {
  const logMsg = payload ? `[${level}] ${message} ${JSON.stringify(payload)}` : `[${level}] ${message}`;
  console.log(logMsg);
  ...
  try {
    ...
    if (error) {
      ...
      logEmitter.emit('log', { id: Date.now().toString(), level, message, timestamp: new Date().toISOString() });
    }
