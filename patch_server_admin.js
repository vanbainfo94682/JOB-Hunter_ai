...
const newApis = `  // ==========================================
  ...
  app.get('/api/system/status', async (req, res) => {
    try {
      const { data } = await supabase.from('SystemConfig').select('*').eq('key', 'maintenanceMode').single();
  ...
  app.post('/api/admin/system/config', requireAuth, async (req, res) => {
    try {
      ...
      await supabase.from('SystemConfig').upsert({ key, value, updatedAt: new Date().toISOString() });
