...
exports.generateUpskillRoadmap = generateUpskillRoadmap;
exports.generateSalaryNegotiation = generateSalaryNegotiation;
 ...
 */
async function generateUpskillRoadmap(profileDetails, targetJobTitle) {
    try {
        await (0, db_1.logSystem)('INFO', `Generating upskill roadmap for ${profileDetails.fullName} targeting ${targetJobTitle}`);
...
async function generateSalaryNegotiation(jobTitle, location) {
    try {
        await (0, db_1.logSystem)('INFO', `Generating salary negotiation script for ${jobTitle}`);
        const prompt = `
