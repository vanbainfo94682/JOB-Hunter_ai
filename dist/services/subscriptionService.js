 ...
 */
async function getOrCreateUserSettings(userId, fallbackKey) {
    ...
    const { data: created, error: createError } = await db_1.supabase
        .from('agent_settings')
        .insert([{
            user_id: userId,
            is_active: false,
...
async function getOrCreateSubscription(userId) {
    const { data: active, error: fetchError } = await db_1.supabase
        .from('subscriptions')
        .select('*')
        .eq('user_id', userId)
        .eq('status', 'ACTIVE')
    ...
    if (active && new Date() > new Date(active.cycle_end)) {
        const { data: expired } = await db_1.supabase
            .from('subscriptions')
    ...
    const { data: created } = await db_1.supabase
        .from('subscriptions')
        .insert([{
            user_id: userId,
            plan_type: 'WEEKLY',
